    ...
    while True:
        ...
        if p_diff != int(difference):
            print("Waiting 59 seconds (" +
                  str(int(difference)).zfill(2) + ")...", end='\r')
...
def make_request(area: str):
    ...
    if response.status_code == 429:
        raise BlockingIOError(
            f"Currently blocked {response.text} waiting for 5 min")
...
def emptyQ():
    success = Logger("success.log")
    error = Logger("error.log")
    while not successQ.empty():
        success.log(successQ.get())
